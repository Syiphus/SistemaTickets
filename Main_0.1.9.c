...
/*int escolheBalcao(int balcoes[]){ // é preciso acabar esta funcao
	int i;
	for(i=0;i<4;i++){
		if(balcao==-1){
		}
...
void recebe(int *ultimoS, ticket _tickets[]){ //falta acabar esta funcao
	int i, _presenca, flag, balcao;
	if(*ultimoS>0){
		do{
			for(i=0; i<*ultimoS; i++){
				flag=0;
				if(_tickets[i].atendido<3){
					//balcao=escolheBalcao(); // retorna o balcao que está isponivel, retorna -1 se estiver tudo ocupado
...
int main () {
	int menu, n, erro=1, erro1=1, erro2=0, ultimoA=0, ultimoB=0, ultimoS=0, ultimoAT=0,balcoes[4];
