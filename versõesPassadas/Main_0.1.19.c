...
void receber(int *ultimoS, int balcoes[], ticket _tickets[],mapas mapa[]){ // ocupa balcoes
								...
								if(!(_tickets[i].letra=='B' && balcaoEscolhido+1==4)){
									...
									if(_presenca==0){ //não está presente
										...
										if(_presenca==1){ //está presente
											...
											_tickets[i].tempo_espe=segundos;
											SetColor(10);
											printf("\nCliente recebido.\n");
											SetColor(15);
											erro=0;
								...
								do{
									...
									switch(menu){
										...
										case 5: //validar se os outros campos foram preenchidos
											...
											mapa[*atendido].tempo_atendimento=segundos-_tickets[i].tempo_espe;
											printf("Esteve a espera %d segundos e o atendimento demorou %d segunoos",mapa[*atendido].tempo_espera,mapa[*atendido].tempo_atendimento);
...
int main () {
	...
	while(testehorario->tm_hour >=8 && testehorario->tm_hour <21)
	{
