...
// linha 60,70,102,103: é preciso retornar o i decrementado, diogo?
		...
		do{
			for(i=0;i<*ultimoS;i++){
				if(_tickets[i].atendido<3){
					...
					_presenca=presenca();
					if(_presenca==1){
						/*i=*/remover(i,_tickets,&*ultimoS);//falta mexer com tempos aqui e guardar info dos que foram atendidos
						break;
					}
					else{
						if(_presenca==0){
							(_tickets[i].atendido)++;// e aqui tbm dos que estao à espera
						}
					}
				}
				else{
					/*i=*/remover(i,_tickets,&*ultimoS);//falta info pros mapas dos que perderam a vez
					break;
				}
			}
		}while(_presenca==0);
...
int remover(int i,ticket _tickets[],int *ultimoS){//remove clientes do vetor
	for(i;i<*ultimoS;i++){
		_tickets[i]=_tickets[i+1];
	}
	(*ultimoS)--;
	//i--;
	//return i; //a funcao retorna o novo valor para i(é preciso?)
}
